...
// Trim will trim the given type array to the given length.
func Trim[T any](input []T, length int) []T {
...
func Bytes5ToInt64(bytes []byte, offset int) int64 {
	return (int64(bytes[offset]&0xff) << 32) |
...
// CopySlice returns a copy of the given type array.
func CopySlice[T any](bytes []T) []T {
	cp := make([]T, len(bytes))
	copy(cp, bytes)
